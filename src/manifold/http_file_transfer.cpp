...
namespace manifold
{
  namespace http
  {
    ...
    future<void> document_root::operator()(server::request req, server::response res, std::smatch matches)
    {
      ...
      else
      {
        ...
        if (!authorized)
        {
          ...
          co_await res.end(status_code_to_reason_phrase(res.head().get_status_code()));
        }
        else
        {
    ...
    future<void> document_root::handle_head(server::response res, std::string file_path)
    {
      struct stat st;
    ...
    future<void> document_root::handle_get(server::response res, std::string file_path)
    {
      struct stat st;
      if (stat(file_path.c_str(), &st) != 0 || (st.st_mode & S_IFREG) == 0)
      {
      ...
      else
      {
        ...
        else
        {
          ...
          while (ifs && res)
          {
            ...
            if (bytes_in_buf > 0)
              co_await res.send(buf.data(), (std::size_t) bytes_in_buf);
          }


          // TODO: Check if res or ifs is bad.
    ...
    future<void> document_root::handle_put(server::request req, server::response res, std::string file_path)
    {
      std::stringstream ss;
      ...
      else
      {
        ...
        while (req && ofs)
        {
          std::size_t amount = co_await req.recv(buf.data(), buf.size());
          ofs.write(buf.data(), amount);
        }

        // TODO: check for bad req
        if (!ofs.good())
        {
          ofs.close();
