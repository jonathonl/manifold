...
namespace manifold
{
  ...
  {
    //----------------------------------------------------------------//
    client::request::request(request_head&& head, const std::shared_ptr<connection::stream>& stream_ptr, const std::string& server_authority) :
      outgoing_message(stream_ptr),
      head_(std::move(head)),
      server_authority_(server_authority)
    {
      if (this->stream_)
      {
//        conn->on_push_promise(stream_id, [conn](request_head&& req, std::uint32_t promised_stream_id) // TODO: handle push promise
//        {
...
//        });
    ...
    //----------------------------------------------------------------//
    future<bool> client::request::send_headers(bool end_stream)
    {
      if (this->head_.method() == "GET" || this->head_.method() == "HEAD")
        end_stream = true;
      if (this->head_.authority().empty())
        this->head_.authority(this->server_authority_);
    ...
    //----------------------------------------------------------------//
    void client::request::on_push_promise(const std::function<future<void>(http::client::request request)>& cb)
    {
//      if (this->connection_) // TODO
//      {
...
//      }
    ...
    //----------------------------------------------------------------//
    future<response_head> client::request::informational_headers()
    {
      header_block tmp = co_await stream_->recv_headers();
      co_return response_head(std::move(tmp));
    }
    //----------------------------------------------------------------//

    //----------------------------------------------------------------//
    future<client::response> client::request::response()
    {
      header_block tmp = co_await stream_->recv_headers(); // TODO: make sure not informational
      co_return client::response(response_head(std::move(tmp)), stream_);
    }
    //----------------------------------------------------------------//

    //----------------------------------------------------------------//
    client::response::response(response_head&& head, const std::shared_ptr<connection::stream>& stream_ptr)
...
//    //----------------------------------------------------------------//
    ...
    future<client::request> client::make_request(const endpoint& ep, request_head head, std::error_code& ec)
    {
      ...
      co_return request(std::move(head), nullptr, "");
    }
    //----------------------------------------------------------------//

    //----------------------------------------------------------------//
    future<std::unique_ptr<connection>> client::secure_connect(const std::string& host, std::uint16_t port, std::error_code& ec)
    {
      std::unique_ptr<connection> ret;

      asio::ip::tcp::resolver::iterator resolve_it = co_await async_resolve(tcp_resolver_, asio::ip::tcp::resolver::query(host, std::to_string(port)), ec);
      if (ec)
      {
      ...
      {
        assert(resolve_it != asio::ip::tcp::resolver::iterator());

        std::cerr << resolve_it->host_name() << std::endl;
        ...
        if (resolve_it == asio::ip::tcp::resolver::iterator())
        {
          ec = std::make_error_code(std::errc::not_connected);
        }
        else
        {
          co_await async_handshake((asio::ssl::stream<asio::ip::tcp::socket>&)sock, asio::ssl::stream_base::client, ec);
          ...
          else
          {
            ...
            std::cerr << "Client ALPN: " << std::string((char*) selected_alpn, selected_alpn_sz) << std::endl;
            if (ec)
            {
              std::cerr << "ERROR: " << ec.message() << std::endl;
            }
            else
            {
              if (std::string((char*) selected_alpn, selected_alpn_sz) == "h2")
              {
                std::size_t bytes_transfered = co_await sock.send(http::connection::http2_preface.data(), http::connection::http2_preface.size(), ec);
                ...
                {
                  ret = std::make_unique<connection>(std::move(sock), http::version::http2);
                }
              }
              else
              {
                ret = std::make_unique<connection>(std::move(sock), http::version::http1_1);
              }
            }
          }
        }
    ...
    //----------------------------------------------------------------//
    future<std::unique_ptr<connection>> client::insecure_connect(const std::string& host, std::uint16_t port, std::error_code& ec)
    {
      std::unique_ptr<connection> ret;
      ...
      else
      {
        ...
        ::manifold::non_tls_socket sock(io_service_);

        while (resolve_it != asio::ip::tcp::resolver::iterator())
        {
          std::error_code conn_ec;
          auto endpoint_to_try = resolve_it++;
          co_await async_connect((asio::ip::tcp::socket&)sock, *endpoint_to_try, conn_ec);
          if (!conn_ec)
            break;
        }
