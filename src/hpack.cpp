...
//      }
    ...
    {
      std::uint32_t msb_code_to_find = 0;
      std::uint8_t bits_to_shift = 32;
      std::uint8_t byte_local_offset = 0;

      while (begin != end)
      {
        std::uint8_t number_of_full_bytes_needed = std::floor(static_cast<float>(bits_to_shift / 8));
        while (bits_to_shift > 0)
        {
          msb_code_to_find << bits_to_shift;
          if (bits_to_shift >= 8)
          {
            if (begin != end)
            {
              msb_code_to_find = msb_code_to_find | (*begin);
              ++begin;
            }
            bits_to_shift -= 8;
          }
      ...
      std::uint8_t current_byte_in_code = 0;
      std::uint8_t bits_filled_in_current_byte = 0;

      while (begin != end)
      {
        int bits_to_shift = ((3 - current_byte_in_code) * 8) - bits_filled_in_current_byte;
        if (bits_to_shift < 0)
        {
          msb_code_to_find |= ((std::uint32_t)*(begin) >> std::abs(bits_to_shift));
        }
        else
        {
          msb_code_to_find |= ((std::uint32_t)*(begin++) << bits_to_shift);
        }

        ++current_byte_in_code;

        if (current_byte_in_code > 3)
        {
          std::map<huffman_code,char>::const_iterator find_result = huffman_code_tree.find(huffman_code(msb_code_to_find, 32));
          if (find_result != huffman_code_tree.end())
          {
            output.push_back(find_result->second);
            msb_code_to_find = msb_code_to_find << find_result->first.bit_length;
            current_byte_in_code = (std::uint8_t)(4 - std::ceil(static_cast<float>(find_result->first.bit_length) / 8));
            bits_filled_in_current_byte = (std::uint8_t)(find_result->first.bit_length % 8);
          }
          else
          {
            break;
            throw std::runtime_error("Huff Code Not Found");
          }
        }
