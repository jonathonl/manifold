...
namespace manifold
{
  namespace http
  {
    ...
    public:
      ...
      {
      public:
        response(response_head&& head, const std::shared_ptr<connection::stream>& stream_ptr);
      ...
      {
      public:
        request(request_head&& head, const std::shared_ptr<connection::stream>& stream_ptr, const std::string& server_authority);
        ...
        void on_push_promise(const std::function<future<void>(http::client::request request)>& cb);
        future<client::response> response();
        future<response_head> informational_headers();
      private:
        request_head head_;
...
//      std::unordered_multimap<endpoint,std::shared_ptr<tls_session>> tls_sessions_;
      ...
      future<std::unique_ptr<connection>> secure_connect(const std::string& host, std::uint16_t port, std::error_code& ec);
      future<std::unique_ptr<connection>> insecure_connect(const std::string& host, std::uint16_t port, std::error_code& ec);
